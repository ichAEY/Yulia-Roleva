...
function openGallery(){if(!galleryOverlay)return;galleryOverlay.hidden=false;lockBody(true);updateSticky()}function closeGallery(){if(!galleryOverlay)return;galleryOverlay.hidden=true;if(lightbox?.hidden!==false)lockBody(false);updateSticky()}
function showLightbox(src){const idx=works.indexOf(src);lightboxIndex=idx>=0?idx:0;if(lightboxImage)lightboxImage.src=`./${works[lightboxIndex]}`;if(lightboxCounter)lightboxCounter.textContent=`${String(lightboxIndex+1).padStart(2,'0')} / ${String(works.length).padStart(2,'0')}`;if(lightbox)lightbox.hidden=false;lockBody(true);updateSticky()}
function stepLightbox(dir){lightboxIndex=(lightboxIndex+dir+works.length)%works.length;showLightbox(works[lightboxIndex])}
function closeLightbox(){if(!lightbox)return;lightbox.hidden=true;if(galleryOverlay?.hidden!==false)lockBody(false);updateSticky()}
...
const stickyWrap=$('#stickyWrap'),hero=$('.mct-hero'),finalBlock=$('.mct-visit-booking');function updateSticky(){if(!stickyWrap||!hero)return;const afterHero=window.scrollY>Math.max(220,hero.offsetHeight*.45),finalVisible=finalBlock?finalBlock.getBoundingClientRect().top<window.innerHeight*.85:false,overlayOpen=galleryOverlay?.hidden===false||lightbox?.hidden===false,visible=afterHero&&!finalVisible&&!overlayOpen;stickyWrap.classList.toggle('is-visible',visible);stickyWrap.setAttribute('aria-hidden',String(!visible))}window.addEventListener('scroll',updateSticky,{passive:true});window.addEventListener('resize',updateSticky);updateSticky();
